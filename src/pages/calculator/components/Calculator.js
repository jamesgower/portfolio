...
class Calculator extends React.Component {
  ...
  onEquationClick = (e) => {
    ...
    if (this.state.previousNums.length > 0) {
      this.setState({
        ...
        currentNumArray: [],
        equation: [
          this.state.previousNums[this.state.previousNums.length - 1],
          [e.target.value],
        ],
        lockEquations: true,
        lockDot: false,
  ...
  onEqualsClick = () => {
    if (this.state.equation !== undefined) {
      ...
      if (
        answer.toString().split("").length > 6 &&
        answer.toString().split("").length < 10
      ) {
        document.getElementById("outcome").classList = "smallerInput";
      ...
      <div className="calculator-container">
        <div id="calculator">
          ...
          <div id="entrybox" className="text-right">
            <div id="outcome">
              {this.state.answer === undefined
                ? this.state.currentNumArray.length > 0
                  ? this.state.currentNumArray.join("")
                  : 0
                : this.state.answer}
            </div>
          ...
          <div id="buttons">
            ...
            <button
              className={
                !this.state.lockEquations
                  ? "orange-btn calcBtn"
                  : "orange-btn calcBtn--disabled"
              }
            ...
            <button
              className="calcBtn"
              onClick={!this.state.lockDot && this.onButtonClick}
              value="."
            >
              .
            </button>
